...
class AlbumDRepository
{
    ...
    {
        return $this->dtAlbumnD->whereIn('id',$id)->delete();
//        return collect(['ServerNo'=>'200','Message'=> trans('message.DeleteSuccess')]);
    }

    public function DeleteAlbum($AlbumId)
    {
        $this->dtAlbumnD->whereIn('album_id',$AlbumId)->delete();
    }
    ...
    public function GetByCondition($request,$columns){
        ...
        return $this->dtAlbumnD->select($columns)
            ->where(function($SubQuery) use ($request) {
                $SubQuery->whereIn('album_id',$request->album_id )
                    ->orwhereRaw("''=IFNULL(?,'')", $request->album_id);
            })->where(function($SubQuery) use ($request) {
                $SubQuery->whereIn('id',$request->id )
