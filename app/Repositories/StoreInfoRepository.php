    ...
    public function GetByCondition($Request,$columns){
        ...
        return $this->StoreInfoRepository->select($columns)
            ->where(function($SubQuery) use ($Request) {
                $SubQuery->where('store_name','=',$Request->store_name)
                    ->orwhereRaw("''=IFNULL(?,'')", [$Request->store_name]);
            })->where(function($SubQuery) use ($Request) {
                $SubQuery->where('local','=',$Request->local)
                    ->orwhereRaw("''=IFNULL(?,'')", [$Request->local]);
            })->where(function($SubQuery) use ($Request) {
                $SubQuery->where('id','=',$Request->id)
                    ->orwhereRaw("''=IFNULL(?,'')", [$Request->id]);
            })->where(function($SubQuery) use ($Request) {
                $SubQuery->where('is_hidden','=',$Request->is_hidden)
                    ->orwhereRaw("''=IFNULL(?,'')", [$Request->is_hidden]);
            })->where(function($SubQuery) use ($Request) {
