...
class AlbumService {
    ...
    public function __construct($strImage=null,$strAlbumName=null,AlbumRepository $AlbumRepository,AlbumDRepository $AlbumDRepository){
    ...
    public function CreateAlbum($Request){
        ...
        if($Request->album_name==null)
        {
            ...
            Storage::makeDirectory('public/'.config('app.album').'/'.$Request->album_name);
            Storage::makeDirectory('public/'.config('app.album').'/'.$Request->album_name.'/thumb');
        }
...
//        Storage::deleteDirectory($this->VirtualStoragePath);
        $this->AlbumDRepository->DeleteAlbum($AlbumId);
...
//            //\Debugbar::info($strFiles);
    ...
    public function RefleshView(){
        ...
        $Carousel = $this->AlbumRepository->GetByCondition($Request,$Columns);
    ...
    {
        return $this->AlbumRepository->ById($id);
    }
    ...
    {
        return $this->AlbumRepository->WhereIn($id);
    }
     ...
     * @return $this
     */
    public function GetByCondition($request,array $columns){

        return $this->AlbumDRepository->GetByCondition($request,$columns);

    }

    public function GetAlbumDByCondition($id,array $columns){

        return $this->AlbumDRepository->getAlbumDByCondition($id,$columns);

    }


    public function CreateAlbumD(array $Data){
        return $this->AlbumDRepository->Create($Data);
    }


    public function GetAlbumName($Id){
        return $this->AlbumRepository->GetAlbumName($Id);
    }
